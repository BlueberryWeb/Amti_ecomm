<div id="editarVendedor-{{$vendedor_pedido->id}}" class="modal fade" tabindex="-1" aria-labelledby="warning-header-modalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            @endphp
            @if ($disponibles)
                <form action="{{route('portal.pedidos.asignar.editar', $vendedor_pedido->id)}}" method="POST">
                    @csrf @method('put')
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-8">
                                <p><b>{{$vendedor->nombre}} {{$vendedor->primer_apellido}} {{$vendedor->segundo_apellido}}</b></p>
                            </div>
                        </div><hr>
                        ...
                        <select required name="vendedor" class="form-select" aria-label="Default select example">
                            ...
                            @foreach ($disponibles as $otro_vendedor)
                                @if ($otro_vendedor->id != $vendedor->id)
